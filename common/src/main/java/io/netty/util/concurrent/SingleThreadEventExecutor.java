...
public abstract class SingleThreadEventExecutor extends AbstractScheduledEventExecutor implements OrderedEventExecutor {
    ...
    private boolean runShutdownHooks() {
        ...
        while (!shutdownHooks.isEmpty()) {
            // 之所以需要拷贝到ArrayList并清理原来的Set，是防止在执行过程中，有新的hook添加进来
            // 拷贝之后并清理掉，后续添加进去的就是新的hook，并且改代码只会被EventLoop线程执行
            // 所以下面两行之间不会出现并发问题导致hook丢失
    ...
    public Future<?> shutdownGracefully(long quietPeriod, long timeout, TimeUnit unit) {
        ...
        for (;;) {
            ...
            // 通过自旋的方式在并发状态下安全地将线程状态修改为ST_SHUTTING_DOWN（正在关闭）
    ...
    protected boolean confirmShutdown() {
        ...
        // 执行所有尚在taskQueue中排队的Task，然后调用shutdown钩子
        ...
        // 如果没有到达指定的超时时间，暂时不退出，每100ms检测一下是否有新任务，有的话继续执行
        // 因为本方法会在一个循环里调用
