...
/**
 * 事件循环核心类，每个都持有一个Selector，并且由一个线程运行该Selector，从始至终都是同样的线程，所以对于其本身是没有并发问题的
...
public final class NioEventLoop extends SingleThreadEventLoop {
    ...
    /**
     * 核心属性 ，有两个是为了解决JDK Selector空轮询假死的问题
     * The NIO {@link Selector}.
     */
    // Netty 优化后的 Selector，对 selectedKeys 做了性能优化，平时使用它来做 I/O。一般是SelectedSelectionKeySetSelector
    private Selector selector;
    // 原生JDK Selector，作为 fallback 用于重建和判断空轮询等场景。
    ...
    protected void run() {
        ...
        for (;;) {
            try {
                ...
                // ShutdownGracefully时会调用到
                // Always handle shutdown even if the loop processing threw an exception.
                try {
                    if (isShuttingDown()) {
                        closeAll();
                        // 调用closeAll方法后，需要通过confirmShutdown看是否能真的退出
                        if (confirmShutdown()) {
                            return;
                        }
                    }
    ...
    private void processSelectedKey(SelectionKey k, AbstractNioChannel ch) {
        ...
        try {
            ...
            if ((readyOps & (SelectionKey.OP_READ | SelectionKey.OP_ACCEPT)) != 0 || readyOps == 0) {
                // NioServerSocketChannel持有的Unsafe会是NioMessageUnsafe类型
                unsafe.read();
            }
    ...
    /**
     * NioEventLoop关闭时执行的操作
     * 会将注册在selector上的所有channel都关闭
     */
    private void closeAll() {
        selectAgain();
        Set<SelectionKey> keys = selector.keys();
        Collection<AbstractNioChannel> channels = new ArrayList<AbstractNioChannel>(keys.size());
        // 关闭注册在selector上的所有channel
        for (SelectionKey k: keys) {
            // 获取SelectionKey上附加的信息
        ...
        // 将selector上所有的AbstractNioChannel关闭
