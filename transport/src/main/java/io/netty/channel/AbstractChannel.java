    ...
    protected abstract class AbstractUnsafe implements Unsafe {
        ...
        private void close(final ChannelPromise promise, final Throwable cause,
            ...
            if (closeExecutor != null) {
                ...
                // 判断当前链路是否有消息正在发送，如果有的话，将SelectionKey的去注册操作封装成Task放到eventLoop中稍后再执行
                if (inFlush0) {
                    invokeLater(new Runnable() {
                        @Override
                        public void run() {
                            // 调用pipeline的fireChannelInactive触发链路关闭通知
                            fireChannelInactiveAndDeregister(wasActive);
                        }
                    });
                } else {
        ...
        public final void write(Object msg, ChannelPromise promise) {
            ...
            int size;
            try {
                // 过滤消息，如果用的不是DirectByteBuf，会将其转为DirectByteBuf
                // 因为直接内存传输效率更好，而且netty自己转换更可控
        ...
        protected void flush0() {
            ...
            try {
                // 流向AbstractNioByteChannel的doWrite方法
